...
- `EngineConfigRepository`：引擎配置仓储接口，定义引擎特定配置存取方法
- `AppConfigRepository`：应用配置仓储接口，定义全局配置存取方法

**基础设施层（infrastructure）** 负责外部服务集成和数据获取：

**engine/ 目录 - 引擎特定实现**：
- `engine/repo/Qwen3TtsVoiceRepository`：通义千问3语音仓储实现，从资源文件加载声音列表
- `engine/repo/Qwen3TtsConfigRepository`：通义千问3配置仓储实现，使用 SharedPreferences 持久化
  - 按引擎 ID 隔离存储不同引擎的配置
  - 存储引擎特定配置：apiKey、voiceId

**app/ 目录 - 应用级全局配置实现**：
- `app/repo/SharedPreferencesAppConfigRepository`：应用配置仓储实现，使用 SharedPreferences
  - 存储应用级全局配置：selected_engine（用户选择的引擎 ID）
  - 与引擎特定配置分离，不与任何特定 TTS 引擎绑定
  - 使用独立的 SharedPreferences 文件 `talkify_app_config.xml`
...
**引擎选择持久化**：
- 使用独立的 [AppConfigRepository] 存储全局配置
- 与引擎特定配置（apiKey、voiceId）分离
- 存储 key：`selected_engine`，存储位置：`talkify_app_config.xml`
- 应用启动时自动恢复上次选择的引擎
- 首次使用时保存默认引擎 ID
- 为后续系统级 TTS 服务调用提供基础
...
| 配置类型 | 仓储接口 | 存储内容 | 示例 |
|---------|---------|---------|------|
| 全局配置 | [AppConfigRepository] | 应用级状态 | 当前选择的引擎 |
| 引擎配置 | [EngineConfigRepository] | 引擎特定数据 | apiKey、voiceId |

### 为什么需要分离？

**问题场景**：如果"选择的引擎"存储在特定引擎的配置中
- ❌ `Qwen3TtsConfigRepository` 持有全局状态
- ❌ 阿里推出新引擎时，命名会造成混淆
- ❌ 架构上不符合单一职责原则
...
| 文件 | 用途 | 管理类 |
|-----|------|-------|
| `talkify_engine_configs.xml` | 引擎特定配置 | `Qwen3TtsConfigRepository` |
| `talkify_app_config.xml` | 全局应用配置 | `Qwen3TtsAppConfigRepository` |
